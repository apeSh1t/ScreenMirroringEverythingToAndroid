...
public class DNSNotify {
    ...
    private static  DNSNotify mInstance;
    public static synchronized DNSNotify getInstance() {
        if (mInstance == null){
            mInstance  = new DNSNotify();
        }
        return mInstance;
    }
    ...
    public void changeDeviceName() {
        mDeviceName = RenderApplication.getInstance().mDeviceInfo.dev_name;
        mMacAddress = RenderApplication.getInstance().mDeviceInfo.mMacAddress;

        mMode = "0x5A7FFFF7,0x1E";
        if (LocalConfigSharePreference.getSettingsVal(RenderApplication.getInstance(),"forceFullScreen").equals("true")) {
            mMode = "0x5A7FFFE4,0x1E";
        }
    }

    //property_get("wfd.airplay.push", pushMode, "1");
    //"0x5A7FFFF7,0x1E"
    //"0x5A7FFFE4,0x1E"
